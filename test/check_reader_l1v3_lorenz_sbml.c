...
#define EXTRA 96
#define SEDML_EXAMPLE "../sed-ml/specification/level-1-version-3/examples/lorenz-sbml/lorenz.xml"
...
int main(void)
{
	...
	assert(strcmp(model->language, "urn:sedml:language:sbml.level-3.version-1") == 0);
	assert(strcmp(model->source, "lorenz-model.xml") == 0);
	...
	assert(strcmp(((const struct sedml_task *)task)->simulationReference, "sim1") == 0);
